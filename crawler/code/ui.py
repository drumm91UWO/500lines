...
def count(*args):
    ...
    ARGS.add_argument(
        'roots', nargs='*',
        default=[urlInput.get()], help='Root URL (may be repeated)')
    ARGS.add_argument(
    ...
    def main():
        ...
        try:
            ...
            reporting.set_words(stringsToCount.get())
...
urlInput = StringVar()
stringsToCount = StringVar()

url_entry = ttk.Entry(mainframe, width=30, textvariable=urlInput)
url_entry.insert(END, "xkcd.com")
url_entry.grid(column=2, row=1, sticky=(W, E))

strings_entry = ttk.Entry(mainframe, width=7, textvariable=stringsToCount)
strings_entry.insert(END, "test")
strings_entry.grid(column=2, row=2, sticky=(W, E))

button = ttk.Button(mainframe, text="Count", command=count, state="enabled")
button.grid(column=3, row=3, sticky=W)

ttk.Label(mainframe, text="Website's url").grid(column=1, row=1, sticky=W)
ttk.Label(mainframe, text="String to look for").grid(column=1, row=2, sticky=E)

for child in mainframe.winfo_children(): child.grid_configure(padx=5, pady=5)

url_entry.focus()
root.bind('<Return>', count)
